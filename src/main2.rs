...
use std::{thread};
use containerd_shim::protos::shim::oci;
...
pub fn prepare_module(mut vm: Vm, spec: &Spec, stdin_path: String, stdout_path: String, stderr_path: String ) -> Result<Vm, WasmRuntimeError> {
    info!("opening rootfs");
    let rootfs_path = oci_spec::runtime::canonicalize_rootfs(spec).to_str().unwrap();
...
impl Instance for Wasi {
    ...
    fn start(&self) -> Result<u32, Error> {
        ...
        info!("vm created");
        let cg = oci_utils::get_cgroup(&spec)?;

        oci_utils::setup_cgroup(cg.as_ref(), &spec)
...
fn main() {
    containerd_shim::run::<ShimCli<Wasi>>("io.container.cwasi.v1", None);
}
