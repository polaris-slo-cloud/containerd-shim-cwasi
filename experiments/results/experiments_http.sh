...
while read -r line; do
  if [[ "$line" =~ "ROUND" ]]; then
    ...
    received_chunk=$(echo "$line" | awk '{print $4}' | sed 's/Z//')
    ...
    receive_duration=$(echo "scale=$scale; $received_chunk_time - $start_transfer_time" | bc)
    serialization_duration=$(echo "scale=$scale; $after_serialization_time - $received_chunk_time" | bc)
    total_duration=$(echo "scale=$scale; $after_serialization_time - $start_transfer_time" | bc)    
